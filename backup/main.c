...
int rtw_power_on(struct rtw_dev *rtwdev)
{
	...
	// 新增入口打印，只要函数被调用就一定会输出
    rtw_err(rtwdev, "===== Enter rtw_power_on() =====");
...
static int rtw_chip_efuse_enable(struct rtw_dev *rtwdev)
{
    ...
    rtw_err(rtwdev, "===== Enter rtw_chip_efuse_enable() =====");
	...
	rtw_err(rtwdev, "===== 111 rtw_chip_efuse_enable() =====");
	rtw_write8(rtwdev, REG_C2HEVT, C2H_HW_FEATURE_DUMP);
	rtw_err(rtwdev, "===== 222 rtw_chip_efuse_enable() =====");
	...
	rtw_err(rtwdev, "===== 333 rtw_chip_efuse_enable() =====");
