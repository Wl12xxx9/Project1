...
// 标记MAC域是否已就绪，就绪后不再重试
// static bool fw_mac_domain_ready = false;
// 总最大等待时间（ms），覆盖8821CS冷启动初始化
#define MAC_READY_TOTAL_TIMEOUT_MS 2000
// 单次重试步长（ms）
#define MAC_READY_RETRY_STEP_MS   100
...
{
	struct rtw_sdio *rtwsdio = (struct rtw_sdio *)rtwdev->priv;
	struct sdio_func *func = rtwsdio->sdio_func;
	bool direct, bus_claim;
	u32 val = 0, retry = 10;
	int ret = 0;
	int waited_ms = 0;
	...
	// 	mdelay(20);
	// }
	// rtw_err(rtwdev, "sdio read32 timeout addr 0x%08x err %d", addr, ret);
	...
	addr = rtw_sdio_to_io_address(rtwdev, addr, direct);
	bus_claim = rtw_sdio_bus_claim_needed(rtwsdio);
	is_high_reg = (addr >= 0x10080);

	// 高偏移寄存器 + 未就绪 = 启用自适应重试等待
	// if (is_high_reg && !fw_mac_domain_ready) {
	if (is_high_reg) {
		while (waited_ms < MAC_READY_TOTAL_TIMEOUT_MS) {
			if (bus_claim)
				sdio_claim_host(rtwsdio->sdio_func);
			...
			// 读写成功，标记就绪，退出重试
			if (ret == 0) {
				// fw_mac_domain_ready = true;
				// rtw_info(rtwdev, "MAC domain ready after %dms", waited_ms);
				rtw_info(rtwdev, "MAC 1111 read reg 0x%x ready after %dms", addr, waited_ms);
				break;
			}

			// 超时则延时后重试
			msleep(MAC_READY_RETRY_STEP_MS);
			waited_ms += MAC_READY_RETRY_STEP_MS;
		}

		// 总超时，打印错误
		if (waited_ms >= MAC_READY_TOTAL_TIMEOUT_MS) {
			rtw_err(rtwdev, "MAC domain init timeout, reg 0x%x", addr);
		}
	} else {
		// 低偏移或已就绪，正常单次读写
		if (bus_claim)
...
static void rtw_sdio_write32(struct rtw_dev *rtwdev, u32 addr, u32 val)
{
	...
	struct sdio_func *func = rtwsdio->sdio_func;
	bool direct, bus_claim;
	u32 dummy = 0, retry = 10;
	int ret = 0;
	int waited_ms = 0;
	bool is_high_reg;

	// while (retry--) {
	// 	dummy = sdio_readl(func, addr, &ret);
	// 	if (!ret)
	// 		return;
	// 	mdelay(20);
	// }
	...
	if (is_high_reg) {
		while (waited_ms < MAC_READY_TOTAL_TIMEOUT_MS) {
			...
			if (ret == 0) {
				// fw_mac_domain_ready = true;
				// rtw_info(rtwdev, "MAC domain ready after %dms (write)", waited_ms);
				rtw_info(rtwdev, "MAC write reg 0x%x ready after %dms", addr, waited_ms);
				break;
			}
		...
		// 总超时，打印错误
		if (waited_ms >= MAC_READY_TOTAL_TIMEOUT_MS) {
			rtw_err(rtwdev, "MAC domain init timeout, write reg 0x%x", addr);
		}
	} else {
		// 低偏移或已就绪，正常单次写入
		if (bus_claim)
		   ...
		   const struct sdio_device_id *id)
{
	...
	rtw_dbg(rtwdev, RTW_DBG_SDIO,
		...
		// ============ 新增开始 ============
	/* Realtek官方workaround：等待MAC电源域稳定，规避-110读写超时 */
	mdelay(500);
	rtw_dbg(rtwdev, RTW_DBG_SDIO, "SDIO probe delay 500ms for MAC power stable");

	/* 禁止SDIO控制器自动掉电，防止MAC域供电丢失 */
	sdio_set_host_pm_flags(sdio_func, MMC_PM_KEEP_POWER);
	// ============ 新增结束 ============
