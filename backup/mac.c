			       ...
			       const struct rtw_pwr_seq_cmd *cmd)
{
	...
	rtw_err(rtwdev, "rtw_pwr_cmd_polling 111\n");
	...
	rtw_err(rtwdev, "rtw_pwr_cmd_polling 222\n");
	...
	rtw_err(rtwdev, "rtw_pwr_cmd_polling 333\n");
...
int rtw_mac_power_on(struct rtw_dev *rtwdev)
{
	...
	rtw_err(rtwdev, "rtw_mac_power_on 111\n");
		
	ret = rtw_mac_pre_system_cfg(rtwdev);
	if (ret)
		goto err;

	rtw_err(rtwdev, "rtw_mac_power_on 600ms delay start!\n");
	// 新增：等待射频电源内部LDO稳定
	mdelay(600);
	rtw_err(rtwdev, "rtw_mac_power_on 600ms delay end!\n");

	ret = rtw_mac_power_switch(rtwdev, true);
	if (ret == -EALREADY) {
		rtw_mac_power_switch(rtwdev, false);
		rtw_err(rtwdev, "rtw_mac_power_on 222\n");

		ret = rtw_mac_pre_system_cfg(rtwdev);
		if (ret)
			goto err;
		
			rtw_err(rtwdev, "rtw_mac_power_on 333\n");
	...
	rtw_err(rtwdev, "rtw_mac_power_on 444\n");

	ret = rtw_mac_init_system_cfg(rtwdev);
	if (ret)
		goto err;
	rtw_err(rtwdev, "rtw_mac_power_on 555\n");
				   ...
				   struct rtw_fw_state *fw)
{
	...
	rtw_err(rtwdev, "__rtw_download_firmware 111\n");
	...
	rtw_err(rtwdev, "__rtw_download_firmware 222\n");

	download_firmware_reg_backup(rtwdev, bckp);
	download_firmware_reset_platform(rtwdev);
	rtw_err(rtwdev, "__rtw_download_firmware 333\n");
	...
	rtw_err(rtwdev, "__rtw_download_firmware 444\n");

	download_firmware_end_flow(rtwdev);

	wlan_cpu_enable(rtwdev, true);
	rtw_err(rtwdev, "__rtw_download_firmware 555\n");
	...
	rtw_err(rtwdev, "__rtw_download_firmware 666\n");
	...
	rtw_err(rtwdev, "__rtw_download_firmware 777\n");
